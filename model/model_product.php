...
function update_pro($pro_id, $pro_name, $pro_price, $amount, $discount, $target_file, $pro_desc, $chat_lieu, $cate_id){
    if($target_file == "../img/"){
        $update_pro = "UPDATE `product` SET pro_name='{$pro_name}', price='{$pro_price}', amount='{$amount}', content = '{$pro_desc}', size = '{$chat_lieu}', discount = '{$discount}', catalog_id = '{$cate_id}' WHERE id = '{$pro_id}'";
    } else{
        $update_pro = "UPDATE `product` SET pro_name='{$pro_name}', price='{$pro_price}', amount='{$amount}', image_link = '{$target_file}', content = '{$pro_desc}', size = '{$chat_lieu}', discount = '{$discount}', catalog_id = '{$cate_id}' WHERE id = '{$pro_id}'";
    }
    connect($update_pro);
}
function add_pro($pro_name, $amount, $pro_price, $target_file, $pro_desc, $giamgia, $chat_lieu, $cate_id){
    $sql_add_pro = "INSERT INTO `product` (`id`, `pro_name`, `price`, `amount`, `image_link`, `content`, `size`,`discount`, `catalog_id`) VALUES (NULL, '{$pro_name}', '{$pro_price}', '{$amount}', '{$target_file}', '{$pro_desc}', '{$chat_lieu}','{$giamgia}','{$cate_id}')";
    connect($sql_add_pro);
}
...
function update_product($pro_id, $amount){
    ...
    $update_pro = "UPDATE `product` SET amount = amount - {$amount} WHERE id = '{$pro_id}'";
    connect($update_pro);
}
